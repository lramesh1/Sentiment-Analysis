...
def tokenize(doc, keep_internal_punct=False):
    ...
    if keep_internal_punct == False:
        return np.array([x.lower() for x in re.findall(r"\w+", doc)])

    if keep_internal_punct == True:
        tokens = []
        for token in [t.lower().strip() for t in re.findall("\w+(?:[-']\w+)*|[-.(]+|\S\w*", doc)]:
...
def lexicon_features(tokens, feats):
    ...
    for i in tokens:
        ...
        feats['neg_words'] = count_neg
        feats['pos_words'] = count_pos
...
def featurize(tokens, feature_fns):
    ...
    for i in feature_fns:
        if i == token_features:
            ...
            token_features(tokens, feats1)
            r.update(feats1)
        if i == lexicon_features:
            feats2 = defaultdict(lambda: 0)
            lexicon_features(tokens, feats2)
            r.update(feats2)
        if i == token_pair_features:
            feats3 = defaultdict(lambda: 0)
            token_pair_features(tokens, feats3)
            r.update(feats3)
    result = sorted(r.items())
    return result
...
def vectorize(tokens_list, feature_fns, min_freq, vocab=None):
    ...
    for token in tokens_list:
        ...
        for f in feats:
            ...
            if (f not in count):
                count[f[0]] += 1
    a.append(cnt)
    # what to do when the vocab is not given and what to do when vocab is given
    if (vocab == None):
    ...
    token = tokens_list
    for i in token:
        for v in sorted(feat[rows]):
            if (v[0] in vocab.keys()):
                idx.append(v[0])
                r.append(rows)
                c.append(vocab[v[0]])
                data.append(v[1])
        rows += 1
        indptr.append(rows)
    X = csr_matrix((data, (r, c)), dtype=np.int64)
        ...
        for m in min_freqs:
            ...
            for p in punct_vals:
                if (p == True):
                    ...
                    X, vocab = vectorize(tokens_list, fi, m)
                    clf = LogisticRegression()
                    accuracy = cross_validation_accuracy(clf, X, labels, k=5)
                    d['punct'] = True
                    d['features'] = fi
                    d['min_freq'] = m
                    d['accuracy'] = accuracy
                    ...
                    s.append(temp2)
    result = r+s
    newlist = sorted(result, key=lambda k: k['accuracy'], reverse=True)
...
def plot_sorted_accuracies(results):
    ...
    newlist = sorted(results, key=lambda k: k['accuracy'])
...
def mean_accuracy_per_setting(results):
    ...
    acc_freq = []
    for d in results:
        if 'min_freq' in d.keys():
            acc_freq.append((d['min_freq'], d['accuracy']))

    for d in results:
        if 'punct' in d.keys():
            acc_freq.append((d['punct'], d['accuracy']))

    for d in results:
        if 'features' in d.keys():
            acc_freq.append((str(d['features']), d['accuracy']))
    d = {}
    for k, v in acc_freq:
    ...
    for k, v in d.items():
        ...
        avg = sum / count
        res.append((avg, k))
    newlist = sorted(res, key=lambda k: k[0])
    return newlist
...
def fit_best_classifier(docs, labels, best_result):
    ...
    fn = best_result['features']
    X, vocab = vectorize(tokens_list, fn, best_result['min_freq'])
    clf = LogisticRegression()
    cv = KFold(len(labels), n_folds=5)
    for train_idx, test_idx in cv:
        clf.fit(X[train_idx], labels[train_idx])
    return clf, vocab
...
def top_coefs(clf, label, n, vocab):
    ...
    cnt = 0;
    index = []
    for a, b in zip(sorted(vocab.keys()), coef):
        if b > 0:
            pos.append((a, b))
            index.append(cnt)
    ...
    if label == 1:
        ...
        return sorted(neg, key=lambda x: x[1], reverse=True)[:n]
...
def parse_test_data(best_result, vocab):
    ...
    X_test, vocab = vectorize(tokens_list, fn, min_freq, vocab)
...
def print_top_misclassified(test_docs, test_labels, X_test, clf, n):
    ...
    for i in range(len(predicted)):
        ...
        if predicted[i] != test_labels[i]:
            sorted_proba[i] = probability
    items = [(v, k) for k, v in sorted_proba.items()]
    items.sort()
    items.reverse()
    sorted_index = [k for v, k in items]
    top_missclassified = {}

    for i in sorted_index[:n]:
        #top_missclassified['labels']=test_labels[i]
        #top_missclassified['predicted'] = predicted[i]
        #top_missclassified['proba'] = predicted_proba[i][predicted[i]]
        print("truth=", test_labels[i], " predicted=", predicted[i], " proba=", predicted_proba[i][predicted[i]])
        print('%s' % test_docs[i])
